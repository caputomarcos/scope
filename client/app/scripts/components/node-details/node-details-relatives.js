...
export default class NodeDetailsRelatives extends React.Component {
  ...
  render() {
    ...
    const limited = relatives && this.state.limit > 0 && relatives.length > this.state.limit;
    const showLimitAction = relatives && (limited || (this.state.limit === 0 && relatives.length > this.DEFAULT_LIMIT));
    const limitActionText = limited ? 'Show more' : 'Show less';
    if (relatives && limited) {
    ...
    return (
      <div className="node-details-relatives">
        {relatives && relatives.map(relative => {
          return <NodeDetailsRelativesLink relative={relative} key={relative.id} />;
        })}
